...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is IntegerType:
            ...
            return "F"
        elif typeIn is DoubleType:
            return "D"
        elif typeIn is ArrayType:
            arr = ""
            for x in range(len(inType.dimensions)):
                arr += "["
            return arr + self.getJVMType(inType.typ) if arr != "" else "[" + self.getJVMType(inType.typ)
    ...
    def emitPUSHDCONST(self, in_, frame):
        # in_: String
        # frame: Frame
        ...
        rst = "{0:.4f}".format(f)
        if rst == "0.0" or rst == "1.0" or rst == "2.0":
            return self.jvm.emitDCONST(rst)
        else:
            return self.jvm.emitLDC2_W(in_)
    ''' 
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        elif type(typ) is DoubleType:
            return self.emitPUSHDCONST(in_, frame)
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) is CharType:
            return self.jvm.emitCALOAD()
        elif type(in_) is BooleanType:
            return self.jvm.emitBALOAD()
        elif type(in_) is FloatType:
            return self.jvm.emitFALOAD()
        elif type(in_) is DoubleType:
            return self.jvm.emitDALOAD()
        elif type(in_) is CharType:
            return self.jvm.emitCALOAD()
        elif type(in_) is StructType or type(in_) is StringType or type(in_) is ArrayType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        frame.push()
        if type(inType) is IntegerType or type(inType) is BooleanType or type(inType) is CharType:
            return self.jvm.emitILOAD(index)
        elif type(inType) is StructType or type(inType) is StringType or type(inType) is ArrayType:
            return self.jvm.emitALOAD(index)
        elif type(inType) is FloatType:
            return self.jvm.emitFLOAD(index)
        elif type(inType) is DoubleType:
            return self.jvm.emitDLOAD(index)
        else:
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        if type(inType) is IntegerType or type(inType) is BooleanType or type(inType) is CharType:
            ...
            return self.jvm.emitDSTORE(index)
        else:
            raise IllegalOperandException(name)
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if type(in_) is IntegerType or type(in_) is BooleanType:
            ...
            result.append(self.jvm.emitDCMPL())
            if op == ">":
                result.append(self.jvm.emitIFLE(labelF))
            elif op == ">=":
                result.append(self.jvm.emitIFLT(labelF))
    ...
    *   After:  ...,value1,value1<p>
    ...
    def emitF2D(self, frame):
        # frame: Frame

        return self.jvm.emitF2D()
    
    def emitI2D(self, frame):
        # frame: Frame

        return self.jvm.emitI2D()
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        if type(in_) is IntegerType or type(in_) is BooleanType or type(in_) is CharType:
            frame.pop()
